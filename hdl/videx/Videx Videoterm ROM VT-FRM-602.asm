;
; Videx Videoterm firmware ROM VT-FRM-602
;
; Disassembly of "Videx Videoterm ROM VT-FRM-602.bin" (1024 bytes)
...
;
; ====================================================================
;   RELATIONSHIP TO ROM 2.4
; ====================================================================
;   VT-FRM-602 differs from ROM 2.4 (50 Hz European) in just 13 bytes,
...
;   full annotation of the unchanged routines; this file mirrors the
;   same labels and only re-comments the SETUP/TABLE region where 602
;   diverges.
;
;   The 13 differences form two logically distinct changes:
;
;     1. CRTC init loop reorganized to count DOWN with DEX/BPL
;        instead of UP with INX/CPX/BNE. Saves one byte in the
;        loop tail; padded with NOP ($EA) at $C82D so $C82E onward
;        stays at the same address as 2.4. The BEQ-skip offset at
;        $C807 shifts from $21 to $20 to track the new SETEXIT
;        location ($C829 in 602 vs $C82A in 2.4). The LDX immediate
;        at $C81A changes from $00 to $0F so that DEX/BPL iterates
;        16 times (X=$0F down through $00).
;          Affected: $C808, $C81A, $C825-$C82D
;
;     2. Two CRTC register init values (TABLE+0 and TABLE+7) are
;        retuned, leaving all other CRTC settings unchanged:
;          R0 (Horizontal Total)  $7A -> $7B   (+1 char clock blanking)
;          R7 (VSYNC Position)    $1D -> $1A   (sync 27 scanlines earlier)
;        Both still produce a 24x80 / 9-scanline-cell / ~50 Hz
;        non-interlaced frame; the tweaks adjust horizontal blanking
;        width and vertical sync placement for a different monitor.
;          Affected: $C8A1, $C8A8
;
; Symbols, conventions, and slot-3 screen-hole assumptions match
; the 2.4 disassembly. Identical instruction streams reproduced
; verbatim for completeness; differences flagged with ";<-- 602".
;
...
;          * load CRTC R15..R0 from TABLE at $C8A1 (count-down)
;          * STA SETAN0    (assert 80-column mode)
;
;   602 vs 2.4: the CRTC init loop is reversed -- 602 starts X at
;   $0F and DEX/BPL down to $FF, then INX restores X=0 before
;   STA CLRAN0. 2.4 starts X at 0 and INX/CPX/BNE up to $10, then
;   STA CLRAN0 directly. Functionally identical (same 16 registers
;   loaded with same table values); the BEQ at $C807 and LDX at
;   $C819 shift one byte to track the new layout, and SETEXIT moves
;   from $C82A (2.4) to $C829 (602). NOP at $C82D pads the saved
;   byte so $C82E (EXIT) stays at the same address.
SETUP:
    $C800:  AD 7B 07          LDA POFF
    $C803:  29 F8             AND #$F8
    $C805:  C9 30             CMP #$30
    $C807:  F0 20             BEQ SETEXIT       ;<-- 602: was F0 21
RESTART:
    $C809:  A9 30             LDA #$30
    ...
    $C819:  A2 0F             LDX #$0F          ;<-- 602: was A2 00
LOOP:
    $C81B:  8A                TXA
    $C81C:  8D B0 C0          STA DEV0          ; CRTC address reg = X
    $C81F:  BD A1 C8          LDA TABLE,X       ; A = init value for reg X
    $C822:  8D B1 C0          STA DEV1          ; CRTC data reg <- value
    $C825:  CA                DEX               ;<-- 602 (was: E8 INX)
    $C826:  10 F3             BPL LOOP          ;<-- 602 (was: E0 10 CPX #$10)
    $C828:  E8                INX               ;<-- 602 (was: D0 F1 BNE LOOP)
SETEXIT:                                        ;<-- 602: SETEXIT moved from $C82A
    $C829:  8D 59 C0          STA CLRAN0        ; assert 80-column
    $C82C:  60                RTS
    $C82D:  EA                NOP               ;<-- 602: padding (was: byte $60)
; ====================================================================
...
INDONE:
    ...
    $C8A0:  60                RTS
; ====================================================================
;   TABLE -- 6845 CRTC initialization values (R0..R15)  (R0,R7 DIFFER)
; ====================================================================
;   Loaded by the SETUP loop, in REVERSE order (R15 first, R0 last)
;   in the 602 variant. Final state of CRTC is identical to writing
;   in forward order. Byte-by-byte comparison vs 2.4:
;
;     R0 =$7B horiz total          (2.4: $7A)  +1 char of H-blank
;     R1 =$50 horiz displayed
;     R2 =$5E horiz sync pos
;     R3 =$2F horiz sync width
;     R4 =$22 vert total
;     R5 =$00 vert total adjust
;     R6 =$18 vert displayed (24)
;     R7 =$1A vert sync pos        (2.4: $1D)  vsync 3 char-rows earlier
;     R8 =$00 interlace mode
;     R9 =$08 max scan line  (9 lines/cell)
;     R10=$E0 cursor start   (cursor off / blink mode 3)
;     R11=$08 cursor end
;     R12=$00 start addr H   (set by code)
;     R13=$00 start addr L   (set by code)
;     R14=$00 cursor pos H   (set by CSRMOV)
;     R15=$00 cursor pos L   (set by CSRMOV)
;
;   Both ROMs target ~50 Hz / 315-scanline non-interlaced. The R0/R7
;   tweaks redistribute blanking time and move VSYNC closer to the
;   end of the displayed field, for a different monitor's preferred
;   sync-to-active porch timing. Frame rate at the same dot clock is
;   ~0.8% slower than 2.4 (124 vs 123 char clocks per scan line).
TABLE:
    $C8A1:  .byte $7B $50 $5E $2F $22 $00 $18 $1A    ;<-- $C8A1 and $C8A8 differ
