...
**NOTE:** This optimization is now internal to `hdl/videx/videx_card.sv` (the
standalone Videx card module). It no longer touches any shared upstream file and
is listed here for historical completeness only.

**File:** `hdl/videx/videx_card.sv` (previously `hdl/video/apple_video.sv`)
...
// Inverse and cursor combined into a single XOR at pixel capture
pix_buffer_r[6:0] <= videxrom_d_r[6:0] ^
    {7{videx_data_r[7] ^ (cursor_active && cursor_byte == 2'd0)}};
```
...
| 4. Charrom halving | videx_card.sv (internal) | Optimization | Low (saves 1 BSRAM) | None | N/A (not upstream) |
...
affected by Fix 1 or Fix 5. Fixes 2 and 3 apply to all boards. Fix 4 is
internal to the Videx card module and not an upstream change.
